...
import { calcHabitsWeekRate, calcHabitsWeekTrend, calcHabitsBadge, calcPerfectDayStreak, calcEveningHabitReminder, calcHabitMilestoneApproachNotify, calcWeeklyReviewReminder, calcPerfectDayMilestoneNotify, calcWeeklyHabitReport, calcMonthlyHabitReport, calcMonthlyPerfectDayReport, calcQuarterlyHabitReport, calcYearlyHabitReport, calcDayOfWeekHabitRates, calcWeakDayOfWeek, calcBestDayOfWeek, calcHabitMorningReminder, calcHabitMomentumCorrelation } from "./lib/habits";
...
export default function App() {
  ...
  // Month-start morning perfect-day count retrospective — fires once per month at 9:00+.
  // Reports the previous month's count of "perfect days" (all habits checked on that day),
  // complementing the average-rate report (monthlyHabitReport) with a full-portfolio signal.
  // monthlyPerfectDayReportDate persists the guard so it fires only once per month-1st even after restart.
  // Design: yesterday.getDate() window mirrors monthlyHabitReport — covers exactly the previous month.
  // Design: date is persisted before the async send (persist-before-send pattern) to prevent duplicates.
  useEffect(() => {
    if (!loaded) return;
    const now = new Date();
