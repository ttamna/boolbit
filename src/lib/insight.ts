...
interface InsightParams {
  ...
  /**
   * Intention-done streak as of yesterday's start — i.e. how many consecutive days BEFORE yesterday
   * the daily intention was marked done. Used to measure the streak length that broke on yesterday's miss.
   * Computed inline in App.tsx (not via calcIntentionDoneStreak) to avoid the 7-day display cap —
   * the badge message "N일 스트릭 끊어짐" needs the accurate length (up to 101 days).
   * When intentionDoneStreak === 0 AND this value ≥ 3 AND !todayIntentionDone,
   * a "intention_done_streak_broken" warning badge fires at priority 10.12.
   * Absent/undefined = not computed by caller; intention_done_streak_broken skipped silently.
   */
  prevIntentionDoneStreak?: number;
  /**
   * All-time highest effective pomodoroGoalStreak value ever recorded; persisted in WidgetData.
   * "Effective" means pomodoroGoalStreak (past days) + 1 (today, when today's goal is met).
   * When effectiveStreak === pomodoroGoalBestStreak AND pomodoroGoalBestStreak > 3 AND the streak is
   * not a POMODORO_GOAL_STREAK_MILESTONE AND sessionsToday >= sessionGoal > 0,
   * the badge shows a "신기록" variant to celebrate a non-milestone personal best.
   * Updated in App.tsx whenever today's sessions reach sessionGoal: max(effectiveStreak, stored best).
   * Absent/undefined = record tracking not available; badge skipped silently.
   */
  pomodoroGoalBestStreak?: number;
  /**
   ...
   * Milestone values 7, 14, 30, 50, 100 trigger a focus_streak_milestone badge when sessionsToday > 0.
   * Absent/undefined = no history or feature not wired by caller; skipped silently.
   */
  focusStreak?: number;
  /**
   * All-time highest focusStreak value ever recorded; persisted in WidgetData.
   * When focusStreak === focusBestStreak AND focusBestStreak > 3 AND the streak is
   * not a FOCUS_STREAK_MILESTONE AND sessionsToday > 0, the badge shows a "신기록" variant
   * to celebrate a non-milestone personal best. Updated in App.tsx whenever a pomodoro session
   * is completed: max(newFocusStreak, stored best).
   * Also used by focus_best_streak_approach (7.4215): when focusStreak < focusBestStreak
   * AND gap ≤ 2 AND sessionsToday > 0, a motivational "역대 최고까지 N일!" nudge fires.
   * Absent/undefined = record and approach badges skipped silently.
   */
  focusBestStreak?: number;
  /**
   * Focus streak as of yesterday's start — i.e. how many consecutive days BEFORE yesterday
   * had at least one pomodoro session. Used to measure the streak length that broke on yesterday's miss.
   * Computed by calling calcFocusStreak(pomodoroHistory, yesterdayHabitsStr) in App.tsx.
   * When focusStreak === 0 AND sessionsToday === 0 AND this value ≥ 3,
   * a "focus_streak_broken" warning badge fires at priority 10.13.
   * Note: sessionsToday === 0 is a defensive guard; in practice focusStreak === 0 already implies
   * sessionsToday === 0 (calcFocusStreak returns ≥ 1 whenever todayCount > 0).
   * Absent/undefined = not computed by caller; focus_streak_broken skipped silently.
   */
  prevFocusStreak?: number;
  /**
   ...
   * When this value hits a milestone (7, 14, 30, 50, or 100), a "momentum sustained" celebration badge fires
   * at priority 10.46 — after pomodoro_today_above_avg (10.45) and before momentum_rise (10.5).
   * Absent/undefined = no history or feature not wired by caller; skipped silently.
   */
  momentumStreak?: number;
  /**
   * All-time highest momentumStreak value ever recorded; persisted in WidgetData.
   * When momentumStreak === momentumBestStreak AND momentumBestStreak > 3 AND the streak is
   * not a MOMENTUM_STREAK_MILESTONE AND todayMomentumQualifies, the badge shows a "신기록" variant
   * to celebrate a non-milestone personal best. Updated in App.tsx whenever a momentum entry is
   * saved: max(newMomentumStreak, stored best).
   * Also used by momentum_best_streak_approach (10.4615): when momentumStreak < momentumBestStreak
   * AND gap ≤ 2 AND todayMomentumQualifies, a motivational "역대 최고까지 N일!" nudge fires.
   * Absent/undefined = record and approach badges skipped silently.
   */
  momentumBestStreak?: number;
  /**
   * Momentum streak as of yesterday's start — i.e. how many consecutive days BEFORE yesterday
   * had a momentum score ≥ 40. Used to measure the streak length that broke on yesterday's miss.
   * Computed by calling calcMomentumStreak(momentumHistory, yesterdayHabitsStr) in App.tsx.
   * When momentumStreak === 0 AND this value ≥ 3,
   * a "momentum_streak_broken" warning badge fires at priority 10.14.
   * Absent/undefined = not computed by caller; momentum_streak_broken skipped silently.
   */
  prevMomentumStreak?: number;
  /**
  ...
  /**
   * All-time highest perfectDayStreak value ever recorded; persisted in WidgetData.
   * When perfectDayStreak === perfectDayBestStreak AND perfectDayBestStreak > 3 AND the streak is
   * not a PERFECT_DAY_MILESTONE AND habitsAllDoneDate === todayStr, the badge shows a "신기록" variant
   * to celebrate a non-milestone personal best. Absent/undefined = record tracking not available.
   */
  perfectDayBestStreak?: number;
  /**
   * All-time highest intentionDoneStreak value ever recorded; persisted in WidgetData.
   * When intentionDoneStreak === intentionDoneBestStreak AND intentionDoneBestStreak > 3 AND the streak is
   * not an INTENTION_DONE_STREAK_MILESTONE AND todayIntentionDone AND todayIntentionDate === todayStr,
   * the badge shows a "신기록" variant to celebrate a non-milestone personal best.
   * Updated in App.tsx whenever intention is marked done: max(current streak, stored best).
   * Absent/undefined = record tracking not available; badge skipped silently.
   */
   ...
   * When |delta| ≤ 2 AND pomodoroPrevWeekSessions ≥ 3,
   * a "pomodoro_week_maintained" info badge fires at priority 10.3835.
   ...
   *   When |delta| ≤ 2 AND pomodoroPrevWeekSessions ≥ 3 AND pomodoroWeekSessions ≥ 1:
   *     "pomodoro_week_maintained" info badge fires at 10.3835.
  ...
  /**
   * Number of consecutive past days (backward from yesterday) on which no daily intention was set.
   * Computed by calcIntentionConsecutiveMiss(intentionHistory, todayStr) in App.tsx.
   * Returns null when intentionHistory has no past entries before today (never-used or today-only) or miss days < 3.
   * When the most recent past entry is outside the 14-day lookback window, returns 14 (capped), correctly
   * signalling a long absence without an unbounded count.
   * When this value is non-null (≥ 3):
   *   - today's intention NOT yet set (todayIntentionDate ≠ todayStr): "intention_gap_warning" badge fires
   *     at priority 10.21, reminding the user to re-engage with their daily intention-setting habit.
   *   - today's intention IS done (todayIntentionDone && todayIntentionDate === todayStr): "intention_recovery"
   *     badge fires within the intention_done block (priority 4.5), celebrating the return after a gap.
   * Absent/undefined = feature not wired by caller; both intention_gap_warning and intention_recovery skipped.
   */
  intentionConsecutiveMissDays?: number;
  /**
   * Number of consecutive past days (ending yesterday) with no pomodoro sessions.
   * Computed by calcFocusDroughtDays(pomodoroHistory, todayStr) in App.tsx.
   * Returns null when pomodoroHistory has no past entries (never used) or drought < 3 days.
   * When this value is non-null (≥ 3) AND sessionsToday === 0:
   *   "focus_drought_warning" badge fires at priority 10.215, prompting re-engagement with daily focus.
   * When this value is non-null (≥ 3) AND sessionsToday > 0:
   *   "focus_recovery" badge fires at priority 6.98, celebrating the user's return to focus sessions.
   * Absent/undefined = feature not wired by caller; both drought badges skipped silently.
   */
  focusDroughtDays?: number;
  /**
   * Pre-computed per-habit bottleneck analysis over a rolling 14-day window.
   * Identifies the single habit with the highest miss rate (>50%) when at least one peer
   * habit has miss rate ≤30%, making it the primary obstacle to perfect days.
   * Computed by calcHabitBottleneck(habits, last14Days) in App.tsx.
   * When present, a "habit_bottleneck" info badge fires at priority 10.225 (after diversity_warning,
   * before almost_perfect_day), surfacing the specific habit name and miss rate.
...
// Chosen at 7/14/30/50/100 to mark one week, two weeks, one month, 50 days, and 100 days of daily focus.
...
export function calcTodayInsight(params: InsightParams): TodayInsight | null {
  const {
    ...
    projectFocusName,
    perfectDayStreak,
    perfectDayBestStreak,
    intentionDoneBestStreak,
    focusBestStreak,
    habitWeekRate,
    habitPrevWeekRate,
  ...
  // Three tiers based on perfectDayStreak:
  //   4.M milestone: streak hits 7/14/30/50/100 → special celebration over generic count
  //   4.C count: streak ≥ 3 → consecutive-day count for added motivation
  //   4.0 generic: streak < 3 or absent → single-day celebration
  if (habitsAllDoneDate === todayStr) {
    if (perfectDayStreak != null && PERFECT_DAY_MILESTONES.includes(perfectDayStreak)) {
      return { text: `🌟 ${perfectDayStreak}일 연속 완벽한 하루 마일스톤! 모든 습관을 ${perfectDayStreak}일 완수했어요`, level: "success" };
    }
    if (perfectDayStreak != null && perfectDayStreak >= 3) {
      // 4.R record tier: non-milestone personal best — more celebratory than the generic count badge.
      // Milestone values are excluded by the early-return above, so no explicit milestone guard needed here.
      const isRecord = perfectDayBestStreak != null
        && perfectDayBestStreak > 3
        && perfectDayStreak === perfectDayBestStreak;
      if (isRecord) {
        return { text: `🌟 완벽한 날 ${perfectDayStreak}일 연속 신기록! 역대 최고 달성`, level: "success" };
      }
  ...
  if (todayIntentionDone && todayIntentionDate === todayStr) {
    ...
    if (intentionDoneStreak != null && intentionDoneStreak >= 3) {
      ...
      // Milestone and month_flawless early-returns above ensure this only fires for non-special streaks.
      // intentionDoneBestStreak > 3 guard: suppresses trivial bests (e.g. bestStreak=2 or 3).
      const isRecord = intentionDoneBestStreak != null
        && intentionDoneBestStreak > 3
        && intentionDoneStreak === intentionDoneBestStreak;
      if (isRecord) {
        return { text: `✨ 의도 달성 ${intentionDoneStreak}일 연속 신기록! 역대 최고 달성`, level: "success" };
      }
      return { text: `✨ ${intentionDoneStreak}일 연속 의도 달성! 실행력이 빛나요`, level: "success" };
    }
    // 4.5.G-R recovery: returning after a ≥3-day gap — more encouraging than the plain generic badge.
    // intentionConsecutiveMissDays ≥ 3 guard mirrors calcIntentionConsecutiveMiss's MIN_INTENTION_MISS_DAYS
    //   threshold so this tier does not fire for trivial 1–2 day gaps even if a caller passes a small value.
    // Reaching here means streak < 3 (gap resets it to 1), so milestone/flawless/streak≥3 did not fire.
    if (intentionConsecutiveMissDays != null && intentionConsecutiveMissDays >= 3) {
      return { text: `✨ ${intentionConsecutiveMissDays}일 만에 의도 달성 재개! 다시 돌아왔습니다`, level: "success" };
    }
  ...
  // habits.length >= 2 guard: a single habit at 7d streak is celebrated by personal_best/milestone badges
  //   (higher priority). Two or more habits all on ≥7d streaks represents meaningful portfolio cohesion.
  // No time-of-day restriction: habit portfolio momentum is relevant context throughout the day.
  // No lastChecked guard: fires on the current persisted streak value regardless of today's check-in
  //   status — mirrors habit_multi_streak (6.95) design. streak_at_risk (priority 1) handles the
  //   "unchecked tonight" urgency case in the evening; this badge is the morning/daytime context view.
  // habit_month_perfect (10.40) yield guard: when all habits satisfy the stricter "perfect this month"
  //   condition (streak ≥ currentMonthDay AND checked today, with currentMonthDay ≥ 14), defer to the
  //   more specific month-level badge. A 15-day perfect-month streak IS a 7d streak; celebrating the
  //   month milestone is more actionable than the generic portfolio-rhythm message.
  // personal_best (11) yield guard: when any habit hits a personal-best milestone TODAY (checked today,
  //   streak ∈ [7/30/100], streak === bestStreak), defer to the more specific milestone celebration.
  ...
  if (habits.length >= 2 && habits.every(h => h.streak >= 7)) {
    ...
    const allPerfectMonth = monthDay >= 14 &&
      habits.every(h => h.lastChecked === todayStr && h.streak >= monthDay);
    const anyAtPersonalBestMilestone = habits.some(
      h => h.lastChecked === todayStr &&
  ...
  // Fires after habit_comeback (6.9): an individual recovery event is rarer and more actionable.
  // Fires AFTER habit_all_streak (6.93): fires only when some (≥3) but not ALL habits are at ≥7d streak.
  ...
  // 6.98. Focus recovery: user returns to pomodoro sessions after 3+ consecutive past days without any.
  // Mirrors intention_recovery (within intention_done block) — celebrates re-engagement after a drought.
  // Fires AFTER habit_multi_streak (6.95): portfolio habit momentum is background context; a focus return is more specific.
  // Fires BEFORE pomodoro_last_one (7): breaking a drought is more significant than a single-session nudge.
  // sessionsToday > 0 guard: fires only when the user has actually started a session today.
  // focusDroughtDays non-null already implies ≥ 3 (calcFocusDroughtDays returns null for shorter droughts).
  // focusDroughtDays absent → skipped silently (not computed by caller or no drought data).
  if (focusDroughtDays != null && sessionsToday > 0) {
    return { text: `🔥 ${focusDroughtDays}일 만에 집중 재개! 다시 시작했어요`, level: "success" };
  }
  ...
  // 7.4101. Pomodoro goal streak record: new all-time best goal streak (non-milestone), today's goal already met.
  // Symmetric sibling of focus_streak_record (7.4205): celebrates a personal best in consecutive goal-met days.
  // pomodoroGoalBestStreak > 3 guard: suppresses trivial bests (e.g. 2-day best from a brief early start).
  // POMODORO_GOAL_STREAK_MILESTONES guard: when effectiveStreak === 7/14/30/50/100, goal_streak_milestone (7.41) fires first.
  // sessionsToday >= sessionGoal guard: mirrors goal_streak_milestone — effective streak includes today only when goal is met.
  // pomodoroGoalBestStreak absent → skipped silently.
  if (
    pomodoroGoalStreak != null &&
    sessionGoal != null && sessionGoal > 0 &&
    sessionsToday >= sessionGoal &&
    pomodoroGoalBestStreak != null && pomodoroGoalBestStreak > 3 &&
    pomodoroGoalStreak + 1 === pomodoroGoalBestStreak &&
    !POMODORO_GOAL_STREAK_MILESTONES.includes(pomodoroGoalStreak + 1)
  ) {
    const total = pomodoroGoalStreak + 1;
    return { text: `🍅 포모도로 목표 ${total}일 연속 신기록! 역대 최고 달성`, level: "success" };
  }

  // 7.42. Focus streak milestone: ≥1 session today AND focusStreak hits 7/14/30/50/100 consecutive focus days.
  ...
  // 7.4205. Focus streak record: new all-time best focus streak (non-milestone), sessionsToday > 0.
  // Symmetric sibling of perfect_day_streak_record (4.R) and intention_done_streak_record (4.5.R).
  // focusBestStreak > 3 guard: suppresses trivial bests (e.g. 2-day best from a brief early attempt).
  // FOCUS_STREAK_MILESTONES guard: when streak === 7/14/30/50/100, focus_streak_milestone (7.42) fires first.
  // sessionsToday > 0 guard: mirrors focus_streak_milestone — streak counts today's contribution only when
  //   today has sessions; streak=10 + sessionsToday=0 means "10 PAST days, today not yet started" → no record.
  // focusBestStreak absent → skipped silently.
  if (
    focusStreak != null && sessionsToday > 0 &&
    focusBestStreak != null && focusBestStreak > 3 &&
    focusStreak === focusBestStreak &&
    !FOCUS_STREAK_MILESTONES.includes(focusStreak)
  ) {
    return { text: `🔥 집중 ${focusStreak}일 연속 신기록! 역대 최고 달성`, level: "success" };
  }

  // 7.421. Focus streak milestone approach: streak is 1–2 days away from the next focus milestone (7, 14, 30, 50, or 100).
  // Motivates the user to maintain their streak before it reaches a celebration-worthy milestone.
  // Fires AFTER focus_streak_milestone (7.42): when streak === 7/14/30/50/100 AND sessionsToday > 0, that badge fires first.
  ...
  // 7.4215. Focus approach personal best: focusStreak is 1–2 days from all-time focusBestStreak.
  // Fires AFTER focus_streak_milestone_approach (7.421): when approaching both a milestone AND a personal best,
  //   the milestone approach fires first. This block only fires for non-milestone personal bests.
  // Fires BEFORE intention_done_streak_milestone_approach (7.422): focus is auto-tracked; this nudge is
  //   a same-domain follow-up to focus_streak_record (7.4205) which fires when streak === best.
  // Mutually exclusive with focus_streak_record (7.4205): record requires focusStreak === focusBestStreak (gap=0);
  //   this block requires focusStreak < focusBestStreak (gap > 0).
  // sessionsToday > 0 guard: mirrors focus_streak_record — streak counts today's contribution only when
  //   today has sessions; without today's session, the streak may not be current.
  ...
  if (
    ...
    focusBestStreak != null && focusBestStreak > 3 &&
    focusStreak < focusBestStreak &&
    (focusBestStreak - focusStreak) <= 2
  ) {
    const gap = focusBestStreak - focusStreak;
    return { text: `🔥 집중 ${focusStreak}일 연속! 역대 최고(${focusBestStreak}일)까지 ${gap}일!`, level: "success" };
  }
  ...
  // 7.425. Perfect day streak milestone approach: perfectDayStreak is 1–2 days away from the next
  // milestone in PERFECT_DAY_MILESTONES (7/14/30/50/100).
  // Completes the milestone-approach series: focus (7.421) > intention (7.422) > pomodoro goal (7.423)
  //   > habit (7.424) > perfect day (7.425). Perfect day is placed last because it is the most demanding
  //   cross-domain metric (all habits every day), so the nudge fires only when none of the individual-domain
  //   approach badges qualify.
  // Natural mutual exclusion with perfect_day (priority 4): when habitsAllDoneDate === todayStr, the
  //   perfect_day block returns first and this block is never reached. No explicit today guard needed —
  //   the approach fires precisely when today's perfect day has NOT yet been achieved, which is the right
  //   moment for a motivational nudge.
  // Structural mutual exclusion with perfect_day_streak_milestone: when perfectDayStreak is in
  //   PERFECT_DAY_MILESTONES (e.g. 7) AND habitsAllDoneDate === todayStr, the milestone badge fires at priority 4; when
  //   today is not yet a perfect day, perfectDayStreak reflects yesterday's streak and
  //   find(m > perfectDayStreak && m - perfectDayStreak ≤ 2) won't return the same milestone (next one
  //   would be ≥ 7 away), so the approach is also structurally suppressed on the milestone day itself.
  // perfectDayStreak absent → skipped silently.
  if (perfectDayStreak != null) {
    const nextPerfectDayMilestone = PERFECT_DAY_MILESTONES.find(m => m > perfectDayStreak && m - perfectDayStreak <= 2);
    if (nextPerfectDayMilestone !== undefined) {
      const daysLeft = nextPerfectDayMilestone - perfectDayStreak;
      return { text: `⭐ 완벽한 하루 ${perfectDayStreak}일 연속! ${daysLeft}일 더 달성하면 ${nextPerfectDayMilestone}일 마일스톤`, level: "success" };
    }
  }
  ...
  // 10.12. Intention done streak broken: intention-done streak (≥3 past days) broke yesterday.
  // intentionDoneStreak === 0: today not done → no current streak.
  // prevIntentionDoneStreak ≥ 3: meaningful streak before the break.
  // !todayIntentionDone guard: if user already marked done today, the streak is alive again.
  // Mirrors pomodoro_goal_streak_broken (10.11) for the intention-done domain.
  if (
    !todayIntentionDone &&
    intentionDoneStreak === 0 &&
    prevIntentionDoneStreak !== undefined && prevIntentionDoneStreak >= 3
  ) {
    return { text: `💔 의도 달성 ${prevIntentionDoneStreak}일 스트릭 끊어짐 — 오늘 다시?`, level: "warning" };
  }

  // 10.13. Focus streak broken: consecutive focus-day streak (≥3 past days) broke yesterday.
  // focusStreak === 0: today has no sessions AND yesterday had none → streak reset.
  // sessionsToday === 0 guard: if user already started a session today, streak alive again.
  // prevFocusStreak ≥ 3: meaningful streak before the break.
  // Mirrors intention_done_streak_broken (10.12) for the focus domain.
  if (
    sessionsToday === 0 &&
    focusStreak === 0 &&
    prevFocusStreak !== undefined && prevFocusStreak >= 3
  ) {
    return { text: `💔 집중 ${prevFocusStreak}일 스트릭 끊어짐 — 오늘 다시?`, level: "warning" };
  }

  // 10.14. Momentum streak broken: consecutive momentum-qualifying streak (≥3 past days) broke yesterday.
  // momentumStreak === 0: yesterday's momentum score < 40 → streak reset.
  // prevMomentumStreak ≥ 3: meaningful streak before the break.
  // Mirrors focus_streak_broken (10.13) for the momentum domain.
  if (
    momentumStreak === 0 &&
    prevMomentumStreak !== undefined && prevMomentumStreak >= 3
  ) {
    return { text: `💔 모멘텀 ${prevMomentumStreak}일 스트릭 끊어짐 — 오늘 다시?`, level: "warning" };
  }
  ...
  // 10.21. Intention gap warning: user hasn't set a daily intention for 3+ consecutive past days.
  // Requires todayIntentionDate !== todayStr: if the user already set today's intention, no warning needed.
  // Fires after habit_consecutive_miss (10.2): habit re-engagement takes priority over intention-setting.
  // Fires before focus_drought_warning (10.215): intention planning enables execution, so it takes priority.
  if (intentionConsecutiveMissDays != null && todayIntentionDate !== todayStr) {
    return { text: `✍️ ${intentionConsecutiveMissDays}일째 의도 미설정 중`, level: "warning" };
  }

  // 10.215. Focus drought warning: no pomodoro sessions for 3+ consecutive past days.
  // Fires after intention_gap_warning (10.21): intention planning takes priority over focus execution.
  // Fires before habit_diversity_warning (10.22): a total execution absence preempts relative imbalance.
  // Guard: sessionsToday === 0 — if the user already started a session today, no drought warning needed.
  if (focusDroughtDays != null && sessionsToday === 0) {
    return { text: `⏸️ ${focusDroughtDays}일째 포모도로 없음`, level: "warning" };
  }
  ...
  // 10.225. Habit bottleneck: one habit has a historically high miss rate over the 14-day window
  // while at least one peer habit is reliable (≤30% miss rate).
  // Pre-computed by calcHabitBottleneck(habits, last14Days) in App.tsx so the insight function
  // stays focused on priority orchestration rather than window-based rate computation.
  // Fires after habit_diversity_warning (10.22): streak imbalance is more immediate/actionable.
  // Fires before almost_perfect_day (10.3): a structural bottleneck insight takes precedence
  // over a single-day "almost there" nudge.
  if (habitBottleneck) {
    return {
      text: `🎯 ${habitBottleneck.icon}${habitBottleneck.name} — 완벽한 날의 가장 큰 장애물 (${habitBottleneck.missRate}% 미완료)`,
      level: "info",
    };
  }
  ...
  // 10.3799. Momentum week record: current 7-entry rolling average from momentumHistory exceeds all
  // previous 7-entry rolling averages. Relies on caller capping momentumHistory to 31 entries.
  // Analogous to pomodoro_week_record (7.495) but placed in the weekly-momentum section (10.3x).
  // Requires ≥ 14 entries (at least 2 complete 7-entry windows for meaningful comparison).
  // Requires currentAvg ≥ 40: a "record" of low scores (e.g. best week avg = 20) is not celebratory.
  // Strictly greater (not ≥): ties are not records.
  // Fires AFTER intention_week_declined (10.374): intention execution pattern checked first.
  // Fires BEFORE momentum_week_strong (10.38): a "new record" is a more specific signal than a
  //   generic tier label — when it fires, it preempts the weekly tier badge for that cycle.
  // momentumHistory absent or < 14 entries → skipped silently.
  if (momentumHistory && momentumHistory.length >= 14) {
    const sorted = [...momentumHistory].sort((a, b) => a.date.localeCompare(b.date));
    // Calculate all 7-entry rolling averages
    const avgs: number[] = [];
    for (let end = 6; end < sorted.length; end++) {
      let sum = 0;
      for (let j = end - 6; j <= end; j++) sum += sorted[j].score;
      avgs.push(sum / 7);
    }
    // With ≥ 14 entries, avgs always has ≥ 8 elements (end runs from 6 to ≥ 13).
    const currentAvg = avgs[avgs.length - 1];
    const prevBest = Math.max(...avgs.slice(0, -1));
    if (currentAvg > prevBest && currentAvg >= 40) {
      return { text: `📈 최근 한 달 최고 주간 모멘텀! (평균 ${Math.round(currentAvg)}점)`, level: "success" };
    }
  }
  ...
  // Fires AFTER momentum_week_record (10.3799): a record preempts the generic tier label.
  ...
  // Fires BEFORE pomodoro_week_maintained (10.3835): absolute improvement outweighs stable consistency signal.
  ...
  // 10.3835. Pomodoro week maintained: rolling 7-day session total within ±2 of the previous 7-day window.
  // Threshold prevWeek ≥ 3: at least 3 sessions last week (≈75 min) as a meaningful consistency baseline;
  //   avoids rewarding "2 this week vs 2 last week" which is too sparse to celebrate.
  // Fires AFTER pomodoro_week_improved (10.383): improvement is strictly better and fires first.
  // Fires BEFORE pomodoro_week_declined (10.384): stable consistency outweighs a minor regression
  //   (a drop ≤ 2 sessions does not meet the "meaningful decline" threshold of 3).
  // Note: prevWeek ≥ 3 AND |delta| ≤ 2 algebraically guarantee curWeek ≥ 1 (cur ≥ prev−2 ≥ 1).
  // Symmetry note: parallels habit_week_maintained (10.351), intention_week_maintained (10.3725), and
  //   momentum_week_maintained (10.3802) — all four domains now have a maintained-level badge.
  // Absent params → skipped silently.
  if (
    pomodoroWeekSessions !== undefined &&
    pomodoroPrevWeekSessions !== undefined &&
    pomodoroPrevWeekSessions >= 3 &&
    Math.abs(pomodoroWeekSessions - pomodoroPrevWeekSessions) <= 2
  ) {
    return { text: `🍅 이번 주 집중 유지! 7일 ${pomodoroWeekSessions}세션`, level: "info" };
  }
  ...
  // Fires AFTER pomodoro_week_maintained (10.3835): a drop ≥ 3 sessions is a genuine regression beyond the maintained range.
  ...
  // 10.46. Momentum streak milestone: consecutive qualifying-momentum days (score ≥ 40) hit 7, 14, 30, 50, or 100 days.
  // Fires AFTER pomodoro_today_above_avg (10.45): a single-day pomodoro achievement is more immediately actionable.
  // Fires BEFORE momentum_rise (10.5): a 7/14/30/50/100-day sustained streak is rarer and more significant than a 3-day rise.
  ...
  // 10.4601. Momentum streak record: new all-time best momentum streak that is NOT a standard milestone.
  // Fires AFTER momentum_streak_milestone (10.46): when streak hits 7/14/30/50/100, that badge fires first.
  // Fires BEFORE momentum_streak_milestone_approach (10.461): a personal best (rare) outweighs an imminent approach nudge.
  // todayMomentumQualifies guard: mirrors momentum_streak_milestone — require evidence of today's qualifying score
  //   before celebrating. Prevents double-fire when calcMomentumStreak falls back to yesterday (score < 40).
  // momentumBestStreak > 3 guard: suppresses trivial bests (e.g. bestStreak=2 from a brief early start).
  // MOMENTUM_STREAK_MILESTONES guard: when streak === 7/14/30/50/100, the milestone badge fires first (already returned).
  //   The guard here is a belt-and-suspenders check in case the block is reached with a milestone value.
  // momentumBestStreak absent → skipped silently.
  if (
    momentumStreak != null &&
    momentumBestStreak != null &&
    momentumBestStreak > 3 &&
    momentumStreak === momentumBestStreak &&
    !MOMENTUM_STREAK_MILESTONES.includes(momentumStreak) &&
    todayMomentumQualifies
  ) {
    return { text: `⚡ 모멘텀 ${momentumStreak}일 연속 신기록! 역대 최고 달성`, level: "success" };
  }

  // 10.461. Momentum streak milestone approach: streak is 1–2 days away from the next milestone (7, 14, 30, 50, or 100).
  // Motivates the user to maintain their streak before it reaches a celebration-worthy milestone.
  // Fires AFTER momentum_streak_milestone (10.46): when streak === 7/14/30/50/100 AND today qualifies, that badge fires first.
  ...
  // 10.4615. Momentum approach personal best: momentumStreak is 1–2 days from all-time momentumBestStreak.
  // Fires AFTER momentum_streak_milestone_approach (10.461): when approaching both a milestone AND a personal best,
  //   the milestone approach fires first. This block only fires for non-milestone personal bests.
  // Fires BEFORE momentum_rise (10.5): approaching a personal best is a more specific motivational signal.
  // Mutually exclusive with momentum_streak_record (10.4601): record requires momentumStreak === momentumBestStreak (gap=0);
  //   this block requires momentumStreak < momentumBestStreak (gap > 0).
  // todayMomentumQualifies guard: mirrors momentum_streak_record — require evidence of today's qualifying score
  //   before celebrating. Prevents double-fire when calcMomentumStreak falls back to yesterday (score < 40).
  // momentumBestStreak > 3 guard: suppresses trivial bests (e.g. bestStreak=2 from a brief early start).
  // momentumBestStreak absent → skipped silently.
  if (
    momentumStreak != null && todayMomentumQualifies &&
    momentumBestStreak != null && momentumBestStreak > 3 &&
    momentumStreak < momentumBestStreak &&
    (momentumBestStreak - momentumStreak) <= 2
  ) {
    const gap = momentumBestStreak - momentumStreak;
    return { text: `⚡ 모멘텀 ${momentumStreak}일 연속! 역대 최고(${momentumBestStreak}일)까지 ${gap}일!`, level: "success" };
  }
  ...
  // 10.505. Momentum sustained peak: 3 consecutive days all with score ≥ 65 (high tier) — sustained excellence.
  // Counterpart to momentum_maintained (10.51): maintained fires when stable at ≥ 40 (mid tier); this fires
  //   when all 3 days are in the HIGH tier (≥ 65), a higher bar reserved for top-tier performance.
  // Fires AFTER momentum_rise (10.5): a strictly rising trend yields to the more dynamic upswing message.
  // Fires BEFORE momentum_maintained (10.51): all-high-tier plateau outranks a mid-tier stable plateau.
  // No calcMomentumTrend dependency: checks raw scores directly so it fires for both stable and
  //   non-monotone high-tier days (e.g., 72→68→71 — not strictly rising but all ≥ 65).
  // momentum_decline (9.5) preempts this block for strictly declining trends at much higher priority.
  // momentumHistory absent → skipped silently.
  if (momentumHistory && momentumHistory.length >= 3) {
    const peakMap = new Map<string, number>(momentumHistory.map(e => [e.date, e.score]));
    const [pyr, pmo, pday] = todayStr.split("-").map(Number);
    const peakScores: number[] = [];
    for (let back = 2; back >= 0; back--) {
      const d = new Date(pyr, pmo - 1, pday - back);
      const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
      peakScores.push(peakMap.get(key) ?? 0);
    }
    if (peakScores.every(s => s >= 65)) {
      return { text: "🔥 3일 연속 고점 모멘텀 유지 중!", level: "success" };
    }
  }
  ...
  // Interaction with momentum_streak_milestone (10.46): when momentumStreak ∈ [7, 14, 30, 50, 100] AND
  ...
  //     Day 50, streak=50: milestone fires; flawless suppressed (50-day months don't exist, so this only
  //       applies in the academic sense — a 50-day momentum streak crosses month boundaries).
  ...
  // 11.15. Habit milestone ETA: forward-looking probability forecast for the next habit streak milestone.
  // Uses geometric survival model (p^n) where p = daily success rate from checkHistory over 14-day window.
  // For each habit with checkHistory, computes the probability of sustaining the streak to the next milestone
  // in HABIT_STREAK_MILESTONES [7, 14, 30, 50, 100]. Picks the habit with the highest probability (most
  // encouraging forecast) when multiple qualify.
  // calcMilestoneETA returns null when daysRemaining < 3 (covered by approach badges at 7.424) or
  //   probability < 10% (unrealistic forecast). Milestone ETA therefore complements approach badges:
  //   approach = "you're almost there!" (1–2 days), ETA = "at your pace, here's when you'll get there" (3+ days).
  // Fires AFTER intention_streak (11.1): a consistent daily-intention signal outweighs a probabilistic forecast.
  // Fires BEFORE final null: on quiet days with no urgent badge, this provides forward-looking motivation
  //   instead of an empty badge slot.
  // habits empty or no habits with checkHistory → skipped silently.
  if (habits.length > 0) {
    const etaWindow = calcLastNDays(todayStr, 14);
    const MILESTONES = HABIT_STREAK_MILESTONES.map(at => ({
      at,
      badge: at >= 100 ? "💎" : at >= 30 ? "⭐" : "🔥",
    }));

    const candidates = habits
      .filter(h => h.checkHistory && h.checkHistory.length > 0)
      .map(h => {
        const rate = calcDailySuccessRate(h.checkHistory, etaWindow);
        if (rate == null || rate === 0) return null;
        const eta = calcMilestoneETA(h.streak, MILESTONES, rate);
        if (!eta) return null;
        return { name: h.name, ...eta };
      })
      .filter((c): c is { name: string; target: number; daysRemaining: number; probability: number; badge: string } => c !== null)
      .sort((a, b) => b.probability - a.probability);

    if (candidates.length > 0) {
      const best = candidates[0];
      const pct = Math.round(best.probability * 100);
      return { text: `🎯 ${best.name} ${best.target}일 ${best.badge}까지 ${best.daysRemaining}일, ${pct}% 확률`, level: "info" as InsightLevel };
    }
  }
