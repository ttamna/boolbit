...
interface InsightParams {
   ...
   *   "habit_month_improved" success badge fires at priority 10.413 (after habit_month_excellent).
   ...
   * habit_month_perfect (10.40) and habit_month_excellent (10.41) preempt via early return.
   ...
   *   so both comparison blocks are only reached when the current avg is below 65.
   ...
   * Both fire before pomodoro_week_improved (10.383); perfect preempts excellent.
   ...
   *   "intention_month_improved" success badge fires at priority 10.4301.
   * When intentionPrevMonthDoneRate - intentionMonthDoneRate ≥ 10:
   *   "intention_month_declined" warning badge fires at priority 10.4302.
   * Absent/undefined = insufficient data; badges skipped silently.
   */
   ...
   * When intentionMonthDoneRate - intentionPrevMonthDoneRate ≥ 10:
   *   "intention_month_improved" success badge fires at priority 10.4301, after intention_month_excellent (10.43).
   * When intentionPrevMonthDoneRate - intentionMonthDoneRate ≥ 10:
   ...
   * Both fire after momentum_month_strong (10.44) and before pomodoro_today_above_avg (10.45);
   ...
   * Both fire after pomodoro_week_goal_excellent (10.3822) and before pomodoro_week_improved (10.383).
   * improved and declined are mutually exclusive; at most one fires per call.
   * Absent/undefined = sessionGoal not set or not computed; badges skipped silently.
   */
...
export function calcTodayInsight(params: InsightParams): TodayInsight | null {
  ...
  //   momentumWeekAvg7d < 65 → lets momentum_week_strong (10.38) fire when momentum was high-tier.
  // Fires AFTER almost_perfect_day (10.3): an actionable "N habits left today" nudge takes precedence
  ...
  // Fires AFTER intention_week_excellent (10.372): excellent/perfect fire first when doneRate ≥ 70,
  //   so this block is only reached when the current rate is below 70 — no explicit guard needed.
  ...
  // Fires AFTER momentum_week_strong (10.38): when avg7d ≥ 65 the strong badge already fired,
  //   so we only reach 10.381 when the current avg is below 65.
  ...
  // Fires AFTER pomodoro_week_goal_excellent (10.3822): when current ≥ 5, excellent already returned above,
  //   so this block only reached when current < 5 — no explicit guard needed.
  ...
  // Fires AFTER habit_month_excellent (10.41): that badge is based on per-habit streak/lastChecked,
  //   not on habitMonthRate — if it fired (early return), this block is never reached.
  ...
  // By the time this block is reached, intentionMonthDoneRate < 80 (perfect/excellent returned above),
  //   so intention_month_improved signals meaningful progress from a lower baseline.
  // Fires AFTER intention_month_excellent (10.43): perfect and excellent preempt the comparison badge.
  ...
  // Fires AFTER momentum_month_strong (10.44): when current avg ≥ 65, strong fires first and returns above.
