...
interface InsightParams {
   ...
   * When this value is ≥ 90, a "great week" celebration badge fires at priority 10.35, after
   ...
   * Computed by calcHabitsWeekRate(habits, last14Days.slice(0, 7)) in App.tsx.
   * last14Days = [today-13, …, today]; slice(0, 7) = [today-13, …, today-7] = days 7–13 ago.
   * Used alongside habitWeekRate to detect a week-over-week improvement of ≥ 10 pp.
   * When habitWeekRate - habitPrevWeekRate ≥ 10 AND habitWeekRate < 90, a "habit_week_improved"
   * badge fires at priority 10.36 (after habit_week_excellent).
...
export function calcTodayInsight(params: InsightParams): TodayInsight | null {
  ...
  //   (7/30/100d), personal_best (11) takes over. For non-milestone bestStreak values,
  //   habit_streak_record (11.01) fires at equality — comeback is suppressed because the user
  //   has fully recovered and a record badge is more precise than a recovery nudge.
  ...
  // 10.35. Excellent week habit rate: 7-day habit completion rate ≥ 90% — weekly consistency celebration.
  ...
  // 10.36. Habit week improved: current 7-day completion rate is ≥ 10 pp higher than the previous 7-day window.
  // Fires AFTER habit_week_excellent (10.35): a ≥90% rate already fires a stronger celebration badge.
  // habitWeekRate < 90 guard is satisfied implicitly: if ≥90, habit_week_excellent returned above.
  // Fires BEFORE pomodoro_today_above_avg (10.45): week-level habit consistency is a broader signal than
  //   a single day's pomodoro count.
  // Both rates must be present — absent prev rate means no comparison basis; badge skipped silently.
  // Threshold 10 pp: avoids noise from small fluctuations (e.g. rounding artifacts at the same effective rate).
  if (
    habitWeekRate !== undefined &&
    habitPrevWeekRate !== undefined &&
    habitWeekRate - habitPrevWeekRate >= 10
  ) {
    return { text: `📈 이번 주 습관 완료율 ${habitWeekRate}%! 지난 주보다 올랐어요`, level: "success" };
  }
  ...
  // 11.01. Habit streak record: habit checked in today, streak equals all-time bestStreak at a non-milestone value.
  // Fires when the user has tied or set a new personal best that is NOT a fixed milestone (7/30/100d) —
  //   those values are already covered by personal_best (11) and would never reach this block.
  // bestStreak > 3 guard: trivial short streaks (1–3 days) are unremarkable.
  // lastChecked === todayStr guard: celebrates the day the record is set/matched.
  // Picks the habit with the highest bestStreak when multiple qualify (most impressive record first).
  ...
  if (streakRecord) {
    return { text: `🏆 ${streakRecord.name} ${streakRecord.streak}일 — 역대 최고!`, level: "success" };
  }
