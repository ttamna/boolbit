...
interface InsightParams {
   ...
   * When this value is non-null (≥ 3) and today's intention is not yet set (todayIntentionDate ≠ todayStr),
   * an "intention_gap_warning" badge fires at priority 10.21, reminding the user to re-engage
   * with their daily intention-setting habit.
   * Absent/undefined = feature not wired by caller; intention_gap_warning skipped silently.
   */
...
export function calcTodayInsight(params: InsightParams): TodayInsight | null {
  ...
  // todayIntentionDate === todayStr guard prevents stale done state from a previous day from firing.
  // Five tiers based on intentionDoneStreak (milestone checked first):
