...
export function calcTodayInsight(params: InsightParams): TodayInsight | null {
  ...
  // Fires BEFORE dual_win (3.95): a full three-domain win is the most celebratory state possible.
  ...
  // sessionGoal > 0 guard: mirrors dual_win — goal=0/absent means no configured goal; falls through to dual_win
  //   when intention conditions fail but sessionGoal is present; falls through to perfect_day when sessionGoal absent.
  // Falls through to dual_win (3.95) when intention is not done or not set today.
  if (
